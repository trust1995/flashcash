...
CURRENT_BLOCK_NUM = 19720809
BLOCK_NUM_1_YRS_AGO = 18608280
BLOCK_JUMP_SPEED = 200
            ...
            if 'message' not in response or response['message'] not in ('OK', "No records found"):
                ...
                for event in response['result']:
                    if flash_event.is_indexed:
                        ...
                        if key == '0xb69ec332f49b7c4d2b101f93c3bed':
                            i = 555
...
def dump_receivers(receivers):
    ...
    with open('receivers_data_arbi_{}.json'.format(datetime.datetime.now().strftime('%y-%m-%d')),'w') as fileobj:
            ...
            while start_block > BLOCK_NUM_1_YRS_AGO or last_round:
                ...
                if num_results >= 1000:
                    ...
                    if earliest_reached == start_block:
                        ...
                        if num_results == 0:
                            earliest_reached = start_block = start_block - BLOCK_JUMP_SPEED * 300
                        else:
                            earliest_reached = start_block = start_block - BLOCK_JUMP_SPEED * int((300.0 / num_results))
